...
test('猎物：伤害 +目标减益层数；洞察：敌意图攻击时翻倍', () => {
  ...
  assert.equal(e.hp, hp - 30);                        // 打击 6 + 猎物(3 层 × 4×等级2 = 24)
...
test('弱点爆破：消耗目标减益、按层造伤', () => {
  ...
  assert.equal(e.hp, hp - 24);                        // 3 层 × 8×等级1
...
test('收割：击杀敌人给永久力量', () => {
  ...
  assert.equal(b._reaping, 4);                        // 2×等级
  b.enemies[0].hp = 0; b._checkEnd();                 // 击杀 → +4 力量
  assert.equal(b.player.statuses.strength, 4);
});
