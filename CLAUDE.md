...
- **任何 commit 之前必须先跑 `npm test`，全部用例全绿（当前 41 例）才允许提交。** 红 / 跳过都不许提交。
...
- **地图＝棋盘格小层**：每个大层 `act`（1..3，定敌人池/数值/场景）含 3 个小层 `floor`：第 1/2 小层为普通棋盘（`genNormalFloor`：入口→两只必经小怪(割点)→出口，精英/商店/祭坛挂死路支线可绕过），第 3 小层为首领一本道（`genBossFloor`）。`grid={type,cols,rows,rooms,entrance,exit}`，房间 `{id,gx,gy,type,done}`，正交相邻即连通；玩家在格间走动，`selectNode` 踩到未通过内容房即触发、踩出口 `_descend` 进下一小层、击败首领 `_nextAct` 进下一大层。顶栏/副标题显示全局楼层 `(act-1)*3+floor`（共 9）。
...
- **改了任何 `js/` 或 `css/` → 必须把 `index.html` 里对应的 `?v=NN` 版本号全部 +1**（无构建的静态站靠 query 串破浏览器缓存；当前 `v=47`）。

## 改内容 / 调平衡的位置

- 数值/经济：`js/data/config.js`（含 `map.floorsPerAct`）　｜　词条：`js/data/affixes.js`　｜　宝石生成规则：`js/data/cards.js` 的 `rollGem`。
- 地图布局/小层结构：`js/engine/run.js` 的 `genNormalFloor` / `genBossFloor`；地图渲染：`js/ui/screens.js` 的 `showMap`（CSS `.map-grid`/`.map-cell`）。
