...
(function (CG) {
  ...
  const PT = [
    ...
    ['curse',    'hex',      '诅咒',   '🪦', '对敌人施加 10 层咒言', atomAct('curse', 10)],
    ...
    ['divine',   'foresee',  '预见',   '🌠', '生成 2 张洞悉到抽牌堆', atomAct('makePeek', 2)],
    ...
    ['arc',      'surge',    '涌流',   '⚡', '获得 4 点电力', atomAct('power', 4)],
    ...
    ['veg',      'sprout',   '采蔬',   '🥬', '生成 1 张素菜', atomAct('food_veg', 1)],
    ['meat',     'hunt',     '狩猎',   '🍖', '生成 1 张荤菜', atomAct('food_meat', 1)],
    ['season',   'spice',    '调味',   '🧂', '生成 1 张调料', atomAct('food_season', 1)],
    ['fire',     'ignite',   '火种',   '🔥', '给敌人附 1 层火（叠加可触发反应）', setAura('fire')],
    ['water',    'douse',    '水沫',   '💧', '给敌人附 1 层水（叠加可触发反应）', setAura('water')],
    ['thunder',  'jolt',     '电荷',   '🌩️', '给敌人附 1 层雷（叠加可触发反应）', setAura('thunder')],
    ['ice',      'rime',     '霜花',   '❄️', '给敌人附 1 层冰（叠加可触发反应）', setAura('ice')],
  ];
