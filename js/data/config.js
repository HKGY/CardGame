...
CG.CONFIG = {
  ...
  startGold: 99,
  acts: 3,                       // 总层数
  runPackSize: 28,               // 开局选包目标「内容量」：随机加主题包直到累计 size(价值原子含时点变体计数) 达此量（基础/修饰词包不计）→ ~6-8 个价值主题
  ...
  // Booster pack 选包权重：本局恒用「融合包」（pickPack 直接返回 fusion），故此处留空；
  //   无 run 时（单测/兜底）pickPack 在所有主题里等概率随机选一个。
  packW: {},
