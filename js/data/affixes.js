...
(function (CG) {
  const BUFFS = {
    ...
    prepare:    { name: '准备', color: '#3ad0d0', score: 3, prepare: 1, desc: n => `本回合力量 +${n}`, long: n => `打出后获得 ${n} 点力量，仅持续到本回合结束` },
    ...
    burning: { name: '燃烧', color: '#e87838', score: 3, burnSelect: 1, desc: n => `消耗 ${n} 张手牌`,   long: n => `打出后：选择并消耗 ${n} 张手牌` },
    nirvana: { name: '涅槃', color: '#c79ae0', score: 5, nirvana: 1,    desc: n => `被消耗时打出 ${n} 次`, long: n => `这张牌被消耗时，自动再打出 ${n} 次（每次再结算一遍其效果）` },
    ...
    stockpile: { name: '蓄能', color: '#b6d36a', score: 4, selfStatus: 'prodBlock', flat: 1, desc: () => `每回合格挡 +1`, long: () => `获得 1 层「蓄能」：此后每回合开始获得 1 点格挡（无视等级、可叠加）` },
    compound:  { name: '复利', color: '#d0e078', score: 5, selfStatus: 'prodGrow', flat: 1,  desc: () => `蓄能逐回合 +1`, long: () => `获得 1 层「复利」：此后每回合开始你的「蓄能」自增 1（无视等级、越拖越强）` },
    harvest:   { name: '丰收', color: '#cfe05a', score: 4, harvest: 1,  desc: () => `产出层总和→格挡`,  long: () => `打出后：把当前耕作/蓄能/复利的层数总和化为格挡` },
    irrigate:  { name: '灌溉', color: '#a0d870', score: 4, irrigate: 1, desc: () => `立即产出 1 次`,    long: () => `打出后：立即结算 1 次「每回合产出」（按当前蓄能加格挡、按耕作抽牌）` },
    ...
    emptymind: { name: '空明', color: '#8a90b0', score: 4, emptyMind: 1, desc: n => `空手时数值最多 +${10 * n}`, long: n => `本牌数值额外 +（max(0, 5 − 出牌后手牌数) × ${2 * n}）：手里牌越少加得越多` },
    devote:    { name: '舍身', color: '#b05a7a', score: 4, devote: 1,    desc: n => `失 ${2 * n} 血·造 ${6 * n} 伤`, long: n => `失去 ${2 * n} 点当前生命，对当前敌人造成 ${6 * n} 点伤害` },
    annihilate:{ name: '湮灭', color: '#6a5a8a', score: 4, annihilate: 1, desc: n => `放逐牌堆 ${n} 张·造伤`, long: n => `从抽牌堆顶放逐 ${n} 张牌，对当前敌人造成（实际放逐数 × 5）点伤害` },
    voidecho:  { name: '虚空回响', color: '#7a6fb0', score: 5, voidEcho: 1, desc: n => `空手时数值×${1 + n}`, long: n => `若出牌后手牌为空，本牌伤害与格挡 ×${1 + n}` },
    ...
    furnace:    { name: '熔炉', color: '#d08850', score: 4, build: 'furnace', desc: () => `建造·每回合 +1 力量`, long: () => `建造熔炉：每回合开始 +1 力量（无视等级、受工坊增幅）` },
    workshop:   { name: '工坊', color: '#b0a878', score: 5, build: 'workshop', desc: () => `建造·增幅其它建筑 +1`, long: () => `建造工坊：每座工坊使你其它建筑每次触发效果 +1（无视等级）` },
    demolish:   { name: '拆解', color: '#c8b060', score: 4, demolish: 1, desc: n => `拆 1 建筑·结算 ${n} 次`, long: n => `拆掉你最早的一座建筑，立即结算它 ${n} 次效果` },
  ...
  const DEBUFFS = {
    ...
    // === 生产包·负面 ===
    cropfail: { name: '歉收', color: '#8a8a4a', score: -3, debuff: true, selfStatus: 'prodSkip', cap: 2,   desc: n => `跳过 ${Math.min(2, n)} 次产出`, long: n => `攒下 ${Math.min(2, n)} 次「歉收」（至多 2）：之后每个回合开始跳过一次被动产出，直到耗尽` },
    upkeep:   { name: '养护', color: '#9a7a4a', score: -3, debuff: true, selfStatus: 'prodUpkeep', cap: 2, desc: n => `每回合能量 -${Math.min(2, n)}`, long: n => `获得 ${Math.min(2, n)} 层「养护」（至多 2）：此后每回合开始失去对应能量（常驻）` },
    stagnate: { name: '滞产', color: '#7a8a5a', score: -3, debuff: true, stagnate: 1, desc: n => `蓄能/耕作各 -${Math.min(2, n)}`, long: n => `打出后：你的「蓄能」与「耕作」各 -${Math.min(2, n)}（至多 2、夹 0）` },
    // === 留置包·负面 ===
    ...
    // === 虚无包·负面 ===
    erode:  { name: '蚀骨', color: '#7a6a8a', score: -3, debuff: true, erode: 1,  desc: n => `减最大生命 ${n}`, long: n => `打出后本场最大生命 −${n}（下限 1）` },
    ...
    // === 建造包·负面 ===
    hazard:   { name: '工伤', color: '#9a6a5a', score: -3, debuff: true, hpLoss: 2, desc: n => `自伤 ${2 * n}`, long: n => `打出后失去 ${2 * n} 点生命（施工事故；复用反噬式自伤）` },
   ...
   * ========================================================================= */
  CG.PACKS = {
    basic:    { name: '基础包', icon: '🎴', color: '#cdd2e2', desc: '只含减益的兜底包（正面词条已分散到各主题包）。',
                buffs: [],
    ...
    hunter:   { name: '猎杀包', icon: '🗡️', color: '#c06050', desc: '借敌人虚弱爆发：上减益→引爆/猎物、处决残血、洞察意图、击杀给永久回报。',
                buffs: ['execute', 'prey', 'exploit', 'insight', 'reaping', 'neutralize', 'shatter', 'poison'],
