...
(function (CG) {
  ...
  const RELIC_PACK = {
    spear: 'attack', greenstone: 'defense', thorncrown: 'bastion', iron_bar: 'defense', regen_lump: 'vitality',
    polydactyly: 'basic', little_steven: 'power', battery: 'basic', magic_shroom: 'attack', feather_boot: 'defense',
    babylon: 'selfharm', eternal_heart: 'defense', moms_knife: 'attack', deaths_touch: 'attack', brother_bobby: 'charge',
    penny: 'selfharm', thorns_hat: 'bastion', scholar: 'basic', lantern: 'basic', thickshield: 'defense',
    the_pact: 'selfharm', cancer: 'selfharm', warbanner: 'selfharm', windbanner: 'selfharm', sac_dagger: 'selfharm', soulforge: 'selfharm',
    damocles: 'power', wafer: 'ghost', holy_mantle: 'ghost', insurance: 'vitality',
  };
  ...
  Object.assign(CG.RELICS, {
    ...
    combo_glove: { name: '连击手套', icon: '🥊', pack: 'power', desc: '【价值】每回合开始：对敌人造成 3 点伤害 ×2', onTurnStart: b => { for (let i = 0; i < 2; i++) if (b.enemy && b.enemy.hp > 0) b.dealAttackDamage(b.player, b.enemy, 3); } },
    war_horn: { name: '战争号角', icon: '📯', pack: 'power', desc: '【价值】每回合开始：对所有敌人造成 3 点伤害', onTurnStart: b => b.damageAll(3) },
    prism_lens: { name: '棱镜透镜', icon: '🔆', pack: 'power', desc: '【价值】每回合开始：下一张牌造成双倍伤害', onTurnStart: b => { b.nextCardDmgMult = Math.max(b.nextCardDmgMult || 1, 2); } },
    iron_fist: { name: '铁拳', icon: '👊', pack: 'attack', desc: '【价值】开局 +2 力量', battleStart: b => b.applyStatus(b.player, 'strength', 2) },
    tower_shield: { name: '塔盾', icon: '🛡️', pack: 'defense', desc: '【价值】每回合开始 +4 格挡', blockTurn: 4 },
    // 减益（每回合施加）
    spite_doll: { name: '怨灵娃娃', icon: '🎯', pack: 'attack', desc: '【价值】每回合开始：对敌人施加 2 层易伤', onTurnStart: atomTurn('vulnerable', 2) },
    sap_totem: { name: '颓力图腾', icon: '💧', pack: 'defense', desc: '【价值】每回合开始：对敌人施加 2 层虚弱', onTurnStart: atomTurn('weak', 2) },
    brittle_seal: { name: '脆裂封印', icon: '🧨', pack: 'weaken', desc: '【价值】每回合开始：对敌人施加 2 层脆弱', onTurnStart: atomTurn('frail', 2) },
    venom_fang: { name: '毒牙', icon: '☠️', pack: 'poison', desc: '【价值】每回合开始：对敌人施加 2 层中毒', onTurnStart: atomTurn('poison', 2) },
    ember_brand: { name: '烙印', icon: '📍', pack: 'mark', desc: '【价值】每回合开始：对敌人施加 2 层印记', onTurnStart: atomTurn('mark', 2) },
    grave_bell: { name: '丧钟', icon: '🪦', pack: 'curse', desc: '【价值】每回合开始：对敌人施加 3 层灾厄', onTurnStart: atomTurn('curse', 3) },
    yoke: { name: '重轭', icon: '🔻', pack: 'weaken', desc: '【价值】每回合开始：使敌人失去 1 点力量', onTurnStart: atomTurn('enemyLoseStr', 1) },
    contagion: { name: '疫源', icon: '🦠', pack: 'weaken', desc: '【价值】开局：对敌人各施加 3 层易伤 / 虚弱 / 脆弱', battleStart: b => { if (b.enemy) ['vulnerable', 'weak', 'frail'].forEach(k => b.applyStatus(b.enemy, k, 3)); } },
    // 防御
    aegis_core: { name: '壁障核心', icon: '🔰', pack: 'bastion', desc: '【价值】本场战斗格挡跨回合保留', battleStart: b => { b._blockRetain = true; } },
    barrier_rune: { name: '屏障符文', icon: '✨', pack: 'ghost', desc: '【价值】每场战斗免疫第一次受到的伤害', battleStart: b => { b._immuneHits = (b._immuneHits || 0) + 1; } },
    // 节奏 / 牌库
    free_ticket: { name: '免费票', icon: '🎟️', pack: 'tempo', desc: '【价值】每回合开始：接下来 1 张牌免费打出', onTurnStart: b => { b.freeCards = (b.freeCards || 0) + 1; } },
    top_hat: { name: '高礼帽', icon: '🎩', pack: 'alchemy', desc: '【价值】首回合：生成 1 张带随机宝石的牌', firstTurn: fx('conjure', 1) },
    crystal_ball: { name: '水晶球', icon: '🔮', pack: 'soul', desc: '【价值】首回合：生成 2 张灵魂到抽牌堆', firstTurn: fx('makePeek', 2) },
    wish_charm: { name: '祈愿符', icon: '🌠', pack: 'plan', desc: '【价值】首回合：从抽牌堆选择 1 张加入手牌', firstTurn: fx('wish', 1) },
    scry_lens: { name: '窥镜', icon: '🔭', pack: 'foresight', desc: '【价值】首回合：预见(看牌库顶 3 张、任选丢弃)', firstTurn: b => { if (b._startForesight) b._startForesight(3); } },
    trick_mask: { name: '戏面', icon: '🤝', pack: 'servant', desc: '【价值】首回合：将 1 张手牌变化为「仆从打击」', firstTurn: b => { if (b._startToServe) b._startToServe('strike', 1); } },
    prayer_beads: { name: '念珠', icon: '📿', pack: 'stance', desc: '【价值】首回合：进入「宁静」姿态', firstTurn: b => { if (b._enterStance) b._enterStance('serenity'); } },
    twin_mirror: { name: '双子镜', icon: '🪄', pack: 'smith', desc: '【价值】首回合：复制 1 张随机手牌', firstTurn: fx('duplicate', 1) },
    dumpster_key: { name: '废料钥匙', icon: '📚', pack: 'reboot', desc: '【价值】每回合开始：从弃牌堆取回 1 张牌', onTurnStart: b => { if (b.discardPile.length && b.hand.length < 10) b.hand.push(b.discardPile.pop()); } },
    grindstone: { name: '砂轮', icon: '📈', pack: 'momentum', desc: '【价值】开局：牌库所有攻击牌伤害永久 +1', battleStart: fx('mindblast', 1) },
    pack_rat: { name: '囤积鼠', icon: '📌', pack: 'charge', desc: '【价值】每回合开始：手牌每有一张，+1 格挡', onTurnStart: b => b.gainBlock(b.player, b.hand.length) },
    // 资源 / 引擎
    ...
    seed_pouch: { name: '种子袋', icon: '🌾', pack: 'tech', desc: '【价值】每回合开始：+2 格挡并抽 1 张', onTurnStart: b => { b.gainBlock(b.player, 2); b.drawCards(1); } },
    karma_wheel: { name: '业轮', icon: '⚙️', pack: 'tech', desc: '【价值】每回合开始：现有每回合增益额外结算 1 次', onTurnStart: b => { if (b._everyTurn && b._everyTurn.length) b._resolveEveryBuffs(1, false); } },
    ash_urn: { name: '骨灰瓮', icon: '♨️', pack: 'exhaust', desc: '【价值】每回合开始：获得等同消耗堆牌数的格挡（至多 20）', onTurnStart: b => b.gainBlock(b.player, Math.min(20, b.exhaustPile.length)) },
    // 代价主题
    blood_chalice: { name: '血杯', icon: '🩸', pack: 'selfharm', desc: '【代价】每回合 -2 生命 →【价值】对敌造成 6 伤害', onTurnStart: b => { b.player.hp = Math.max(1, b.player.hp - 2); if (b.enemy && b.enemy.hp > 0) b.dealAttackDamage(b.player, b.enemy, 6); } },
    coin_purse: { name: '钱袋', icon: '💰', pack: 'money', desc: '【价值】每回合开始：+5 金币', onTurnStart: b => { if (b.run) b.run.gold += 5; } },
    adrenal_gland: { name: '肾上腺', icon: '💉', pack: 'burst', desc: '【价值】首回合：对所有敌人造成 8 点伤害', firstTurn: b => b.damageAll(8) },
    voodoo_doll: { name: '巫蛊娃娃', icon: '🎎', pack: 'voodoo', desc: '【价值】每回合开始：你每有 1 层减益，对敌造成 2 伤害', onTurnStart: b => { const d = ['vulnerable', 'weak', 'frail'].reduce((s, k) => s + (b.player.statuses[k] || 0), 0); if (d > 0 && b.enemy && b.enemy.hp > 0) b.dealAttackDamage(b.player, b.enemy, d * 2); } },
    tinker_kit: { name: '百宝箱', icon: '🎲', pack: 'gadget', desc: '【价值】首回合：抽 2 张牌', firstTurn: b => b.drawCards(2) },
    sculptor_chisel: { name: '塑形凿', icon: '🎨', pack: 'shape', desc: '【价值】首回合：生成 1 张惩戒与 1 张平安', firstTurn: b => { CG.Effects.apply(b, { type: 'makeSmite', value: 1 }, b.player); CG.Effects.apply(b, { type: 'makePeace', value: 1 }, b.player); } },
    primordial_egg: { name: '初源之卵', icon: '🥚', pack: 'origin', desc: '【价值】首回合：生成 1 张 β（打出后逐级变化）', firstTurn: b => { if (b._addToHand) b._addToHand(CG.makeFoodCard('beta')); } },
    // 造物 / 药材 / 元素
    ...
    scrap_box: { name: '甲片盒', icon: '🧱', pack: 'scrap', desc: '【价值】首回合：生成 2 张甲片', firstTurn: fx('makeScrap', 2) },
